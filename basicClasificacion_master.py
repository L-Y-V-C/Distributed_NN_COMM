...
for epoch in range(num_epochs):
    ...
    for batch_x, batch_y in train_loader:
        ...
        if epoch > 0:
            slave_w_received = []
            print("\nafter 1st time")
            slave_w_received.append(w_slave1)
            slave_w_received.append(w_slave2)
            avg_weights = {}

            print("slave 1")
            print(w_slave1['fc1.weight'][:2])
            print("slave 2")
            print(w_slave2['fc1.weight'][:2])
            print("\nweights master")
            print(M['fc1.weight'][:2])

            # replace for calculator function
            for key in w_slave1:
                avg_weights[key] = torch.zeros_like(w_slave1[key])
                avg_weights[key] += w_slave1[key]
                avg_weights[key] += w_slave2[key]
                avg_weights[key] /= 2
        ...
        #M = copy.deepcopy(model.state_dict()) # weights
        M = copy.deepcopy(model.state_dict()) # weights
        print("\nweights master")
        print(M['fc1.weight'][:2])

        # change test
        w_slave1 = copy.deepcopy(M)
        w_slave2 = copy.deepcopy(M)
        for key in w_slave1:
            w_slave1[key] += 0.01
        for key in w_slave2:
            w_slave2[key] += 0.02
